...
void camera::move_forward(const float dz) {
  offset_position(dz * forward());
}

void camera::move_right(const float dx) {
  offset_position(dx * right());
}
...
void camera::look_at(const float x, const float y, const float z) {
  view_dirty = true;
  glm::vec3 direction = glm::normalize(glm::vec3{x, y, z} - position);
